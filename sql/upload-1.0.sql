...
DROP INDEX IF EXISTS "t_tenant_user_tenant_id_email_sync_source";
CREATE UNIQUE INDEX IF NOT EXISTS "t_tenant_user_tenant_id_email_sync_source"
...
CREATE TABLE IF NOT EXISTS t_tenant_org_sync (
    ...
    options JSONB,
    tenant_id INT8 NOT NULL REFERENCES t_tenant(id) ON DELETE CASCADE ON UPDATE CASCADE,
...
CREATE TABLE IF NOT EXISTS t_tenant_third_login (
    id SERIAL PRIMARY KEY,
    type VARCHAR(255),
    config JSONB NOT NULL DEFAULT '{}'::jsonb,
    options JSONB,
...
COMMENT ON TABLE t_tenant_third_login IS '第三方登录配置';
COMMENT ON COLUMN t_tenant_third_login.type IS '第三方登录渠道类型';
